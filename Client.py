...
class Send(threading.Thread):
    ...
    def run(self):
        while True:
            ...
            if message == 'QUIT':
                self.sock.sendall('Server: {} is out.'.format(self.name).encode('ascii'))
                break
...
class Client:
    ...
    def start(self):
        ...
        print()
        print('Welcome, {}! Almost ready to make friends...'.format(self.name))
        ...
        self.sock.sendall('Server: {} is up in here. Say hi!'.format(self.name).encode('ascii'))
        print("\r Got to go type 'QUIT' at anytime\n")
        print('{}: '.format(self.name), end = '')
...
def main(host, port):
    ...
    window = tk.Tk()
    window.title('NJ Chatroom')


    frm_messages = tk.Frame(master=window,)
    scrollbar = tk.Scrollbar(master=frm_messages, bg='Black')
    messages = tk.Listbox(
        master=frm_messages, 
    ...
    text_input.insert(0, "Start a friendship here...")
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='NJ Chatroom')
